...
export default function Dashboard() {
  ...
  const parseProjectData = (
    ...
    console.log(
      "[PROJECT] Starting to parse project data from output:",
      output.length,
    );
        ...
        if (columns.length >= 5) {
          ...
          if (budgetColumn && budgetColumn.includes("$")) {
            ...
            if (!isNaN(budgetValue)) {
              ...
              projectCount++;
              console.log(
                `[PROJECT] Found project with budget: $${budgetValue.toLocaleString()}`,
              );
            }
          }
        }
    ...
    console.log(
      `[PROJECT] Parsed ${projectCount} projects with total budget: $${totalBudget.toLocaleString()}`,
    );
    return { count: projectCount, totalBudget };
  };

  const parseProjectsList = (
    output: string,
  ): { id: number; name: string; budget: number }[] => {
    const lines = output.trim().split("\n");
        ...
        if (columns.length >= 5) {
          ...
          if (idColumn && nameColumn && budgetColumn) {
            ...
            if (!isNaN(id) && !isNaN(budgetValue)) {
              ...
              console.log(
                `[SUCCESS] Parsed project: ID=${id}, Name="${nameColumn}", Budget=$${budgetValue.toLocaleString()}`,
              );
            } else {
              console.log(
                `[WARN] Could not parse ID or Budget from line: ${JSON.stringify(line)}`,
              );
            }
          } else {
            console.log(
              `[WARN] Could not parse ID, Name, or Budget from line: ${JSON.stringify(line)}`,
            );
          }
        } else {
          console.log(
            `[WARN] Line has insufficient columns: ${JSON.stringify(line)}`,
          );
        }
      }
    }
    ...
    console.log(
      `[PROJECTS] Parsed ${projects.length} projects for expense modal`,
    );
    return projects;
  };
  ...
  const parseAllExpensesTotalSpent = (output: string): number => {
    ...
    console.log(
      "[EXPENSE] Parsing total expenses from output length:",
      output.length,
    );
          ...
          if (column.includes("$")) {
            ...
            if (expenseMatch) {
              const expenseAmount = parseFloat(
                expenseMatch[0].replace(/[$,]/g, ""),
              );
              if (!isNaN(expenseAmount)) {
                totalExpenses += expenseAmount;
                console.log(
                  `[EXPENSE] Found expense: $${expenseAmount.toLocaleString()}`,
                );
              }
            }
          }
    ...
    console.log(
      `[EXPENSE] Total expenses calculated: $${totalExpenses.toLocaleString()}`,
    );
  ...
  const parseExpensesFromOutput = (output: string): { cost: number }[] => {
    ...
    console.log(
      `[DASHBOARD] Total expenses found: ${expenses.length}, Total amount: $${expenses.reduce((sum, exp) => sum + exp.cost, 0)}`,
    );
    return expenses;
  };
  ...
  const loadDashboardData = async () => {
    try {
      ...
      console.log("[DASHBOARD] Parsed project data:", {
        projectCount,
        totalBudget,
      });
      ...
      if (projectCount > 0) {
        ...
        for (const project of projectsList) {
          try {
            ...
            console.log(
              `[DASHBOARD] Project ${project.id} (${project.name}): $${projectSpent.toLocaleString()}`,
            );
          } catch (err) {
            console.warn(
              `[DASHBOARD] Could not load expenses for project ${project.id} (${project.name}):`,
              err,
            );
          }
        }
      ...
      console.log(
        `[DASHBOARD] Total spent across all projects: $${totalSpent.toLocaleString()}`,
      );
          ...
          <div className="bg-white dark:bg-[#131619] border border-gray-200 dark:border-[#1d2328] rounded-xl p-6 shadow-sm hover:shadow-md transition-all duration-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900 dark:text-white mt-2">
                  {formatLargeNumber(stats.totalBudget)}
                </p>
              </div>
          ...
          <div className="bg-white dark:bg-[#131619] border border-gray-200 dark:border-[#1d2328] rounded-xl p-6 shadow-sm hover:shadow-md transition-all duration-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900 dark:text-white mt-2">
                  {formatLargeNumber(stats.totalSpent)}
                </p>
              </div>
          ...
          <div className="bg-white dark:bg-[#131619] border border-gray-200 dark:border-[#1d2328] rounded-xl p-6 shadow-sm hover:shadow-md transition-all duration-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900 dark:text-white mt-2">
                  {formatLargeNumber(stats.totalBudget - stats.totalSpent)}
                </p>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          <div className="group relative bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 hover:shadow-xl transition-all duration-300 hover:scale-105 hover:-translate-y-1 overflow-hidden">
            ...
            <div className="relative">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-4 text-sm">
                Start tracking a new renovation project with budget and details.
              </p>
              <button
          ...
          <div className="group relative bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 hover:shadow-xl transition-all duration-300 hover:scale-105 hover:-translate-y-1 overflow-hidden">
            ...
            <div className="relative">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-4 text-sm">
                Record materials, labor costs, and other project expenses.
              </p>
              <button
          ...
          <div className="group relative bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 hover:shadow-xl transition-all duration-300 hover:scale-105 hover:-translate-y-1 overflow-hidden">
            ...
            <div className="relative">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-4 text-sm">
                Analyze project performance, costs, and budget utilization.
              </p>
              <button
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400 text-sm mt-1">
                Latest updates from your renovation projects
              </p>
            </div>
            {stats.projectCount > 0 && (
          ...
          {stats.isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-6 max-w-md mx-auto">
                Begin your real estate journey by creating your first renovation
                project. Track expenses, manage budgets, and monitor progress
                all in one place.
              </p>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    ...
                    {stats.projectCount !== 1 ? "s" : ""} actively monitored
                    with {formatLargeNumber(stats.totalBudget)} total investment
                  </p>
                </div>
              </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-xs px-2 py-1 rounded-full ${
                        stats.totalBudget > 0 &&
                        stats.totalSpent / stats.totalBudget > 0.8
                          ? "bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300"
                          : stats.totalBudget > 0 &&
                              stats.totalSpent / stats.totalBudget > 0.6
                            ? "bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300"
                            : "bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300"
                      }`}
                    >
                      {stats.totalBudget > 0
                        ? `${formatPercentage((stats.totalSpent / stats.totalBudget) * 100)}%`
                        : "0%"}
                    </span>
                  </div>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    {formatLargeNumber(stats.totalSpent)} spent of{" "}
                    {formatLargeNumber(stats.totalBudget)} budget -{" "}
                    {stats.totalBudget > 0 &&
                    stats.totalSpent / stats.totalBudget > 0.8
                      ? "Monitor closely"
                      : "On track"}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    ...
                    {stats.projectCount > 0
                      ? " Active management"
                      : " Ready to start"}{" "}
                    •{formatCompactNumber(stats.totalBudget - stats.totalSpent)}
