...
export default function Debug() {
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-50">🔧 PROJECT & EXPENSE TESTING</h1>
      ...
      <div className="mb-8 p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg border border-indigo-200 dark:border-indigo-800">
        <h2 className="text-xl font-semibold mb-4 text-indigo-900 dark:text-indigo-100">🏗️ PROJECT MANAGEMENT MODULE</h2>
        
        {/* STEP A: Project Creation */}
        <div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          <h3 className="font-semibold mb-3">📋 STEP A: Project Creation & Management</h3>
          
          {/* Project Form */}
          <div className="grid grid-cols-3 gap-3 mb-3">
            <div>
              <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Project ID:</label>
              <input
                type="number"
                value={projectId || ''}
                onChange={(e) => {
            ...
            <div>
              ...
              <input
                type="number"
                value={projectBudget || ''}
                onChange={(e) => setProjectBudget(e.target.value ? Number(e.target.value) : undefined)}
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
                  <div>
                    <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Property Type (Create only):</label>
                    <select
                      value={projectType}
                      onChange={(e) => {
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Property Class (Create only):</label>
                    <select
                      value={projectStatus}
                    ...
                    >
                      {projectType === 'single_family' ? (
                        <>
                          <option value="sf_class_a">SF Class A - Ultra-Luxury ($2.5-4M)</option>
                          <option value="sf_class_b">SF Class B - Luxury ($1-2M)</option>
                          <option value="sf_class_c">SF Class C - Mid-Range ($700K-999K)</option>
                          <option value="sf_class_d">SF Class D - Budget (&lt;$550K)</option>
                        </>
                      ) : (
                        <>
                          <option value="mf_class_a">MF Class A - Premium ($1-1.5M)</option>
                          <option value="mf_class_b">MF Class B - Standard ($750K-900K)</option>
                          <option value="mf_class_c">MF Class C - Value ($500K-749K)</option>
                        </>
                      )}
                    </select>
                  </div>
          ...
          <div className="grid grid-cols-4 gap-3">
            <button
              onClick={() => runTest(`Create "${projectName}" Project`, async () => {
            ...
            <button
              onClick={() => runTest("List All Projects", () => TauriService.getProjects())}
              disabled={isLoading}
              className="p-3 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50"
            >
              📊 List Projects
            ...
            <button
              onClick={() => runTest(`Update Project ${projectId}`, async () => {
                if (!projectId) {
                  throw new Error('Project ID is required for update')
                }
                if (!projectName && !projectBudget) {
                  throw new Error('Project name or budget is required for update')
                }
                ...
                return await TauriService.updateProject(projectId, updateData)
              })}
            ...
            <button
              onClick={() => runTest(`Delete Project ${projectId}`, async () => {
                if (!projectId) {
          ...
          <div className="mt-3 p-2 bg-yellow-100 dark:bg-yellow-900/30 rounded border-l-4 border-yellow-500">
            <p className="text-xs text-yellow-800 dark:text-yellow-200">
              📝 <strong>Update Note:</strong> Property Type and Property Class can only be set during creation. 
              Updates only modify Name, Budget, and Description.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-8 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
        <h2 className="text-xl font-semibold mb-4 text-yellow-900 dark:text-yellow-100">💰 EXPENSE MANAGEMENT MODULE</h2>

        {/* STEP 1: Basic Tests */}
        <div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          <h3 className="font-semibold mb-3">📋 STEP 1: Basic Connection Tests</h3>
          <div className="grid grid-cols-2 gap-3">
            <button
              onClick={() => runTest("Check Projects", () => TauriService.getProjects())}
              disabled={isLoading}
              className="p-3 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50"
            >
              📊 List Projects
            ...
            <button
              onClick={() => runTest("Check Rooms in Project 1", () => TauriService.getRooms(1))}
              disabled={isLoading}
              className="p-3 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50"
            >
              🏠 List Rooms
      ...
      <div className="mb-8 p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg border border-purple-200 dark:border-purple-800">
        <h2 className="text-xl font-semibold mb-4 text-purple-900 dark:text-purple-100">📊 REPORTS & ANALYTICS MODULE</h2>
        
        {/* STEP A: Data Collection Tests */}
        <div className="mb-6 p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg">
          <h3 className="font-semibold mb-3">📋 STEP A: Advanced Data Collection</h3>
          
          {/* Data Collection Buttons */}
          <div className="grid grid-cols-3 gap-3 mb-4">
            <button
              onClick={() => runTest("All Projects Analysis", async () => {
            ...
            <button
              onClick={() => runTest("Budget vs Spent Analysis", async () => {
                const projects = await TauriService.getProjects()
            ...
            <button
              onClick={() => runTest("ROI Calculation Test", async () => {
        ...
        <div className="mb-6 p-4 bg-violet-50 dark:bg-violet-900/20 rounded-lg">
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <button
              onClick={() => runTest("Trend Analysis", async () => {
            ...
            <button
              onClick={() => runTest("Cost Variance Report", async () => {
        ...
        <div className="mb-6 p-4 bg-pink-50 dark:bg-pink-900/20 rounded-lg">
          <h3 className="font-semibold mb-3">📄 STEP C: Export & Reporting Features</h3>
          
          <div className="grid grid-cols-3 gap-3">
            <button
              onClick={() => runTest("Generate Summary Report", async () => {
                const projects = await TauriService.getProjects()
                const allExpenses = await TauriService.getAllExpenses()
...
${allExpenses}`
            ...
            <button
              onClick={() => runTest("Project Performance Metrics", async () => {
            ...
            <button
              onClick={() => runTest("Risk Assessment Report", async () => {
                const projects = await TauriService.getProjects()
        ...
        <div className="mt-4 p-3 bg-green-50 dark:bg-green-900/20 rounded border-l-4 border-green-500">
          <h4 className="font-semibold text-green-900 dark:text-green-100 mb-2">✅ SUCCESS CRITERIA FOR REPORTS & ANALYTICS</h4>
      ...
      <div className="mb-6 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
        ...
        <div className="grid grid-cols-3 gap-3 mb-3">
          ...
          <div>
            <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Room Name:</label>
            <input
              type="text"
              value={roomName}
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            onClick={() => runTest(`Add $${expenseCost || 100} expense to ${roomName || 'Living Room'}`, async () => {
              const testExpense = {
          ...
          <button
            onClick={() => runTest("Test Over-Budget Expense", async () => {
              const testExpense = {
                projectId: projectId || 1,
                roomName: roomName || "Living Room",
                category: "material" as const,
      ...
      <div className="mb-6 p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            onClick={() => runTest("List All Expenses", () => TauriService.getExpenses(projectId || 1))}
            disabled={isLoading}
          ...
          <button
            onClick={() => runTest("Check Project Budget", () => TauriService.getBudgetStatus(projectId || 1))}
            disabled={isLoading}
            className="p-3 bg-purple-600 text-white rounded-md hover:bg-purple-700 disabled:opacity-50"
          >
            💰 Check Budget
      ...
      <div className="mt-6 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
        <h3 className="font-semibold text-green-900 dark:text-green-100 mb-2">✅ SUCCESS CRITERIA FOR EXPENSE MODULE</h3>
        <ul className="text-sm text-green-800 dark:text-green-200 space-y-1">
