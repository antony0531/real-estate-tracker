...
import QuickEditDropdown, {
  STATUS_OPTIONS,
  PRIORITY_OPTIONS,
} from "@/components/QuickEditDropdown";
...
export default function Projects() {
  ...
  const loadProjects = async () => {
    try {
      ...
      toast.error(
        `Failed to load projects: ${TauriService.handleError(error)}`,
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const parseProjectsFromOutput = async (
    output: string,
  ): Promise<Project[]> => {
    ...
    for (const line of lines) {
      if (line.startsWith("│") && !line.startsWith("┃")) {
        ...
        if (columns.length >= 7) {
          ...
          const priority = columns[3].toLowerCase() as
            | "high"
            | "medium"
            | "low"
            | "urgent";
          ...
          if (!isNaN(id) && budgetMatch) {
            ...
            try {
              ...
              console.log(
                `[PROJECTS] Raw expenses output for project ${id}:`,
                expensesOutput,
              );
            ...
            console.log(
              `[PROJECTS] Project ${id} completion: ${completion}% (spent: $${spent}, budget: $${budget})`,
            );
            ...
            console.log(
              `[PROJECTS] Added project ${id} with spent: $${spent}, budget: $${budget}, completion: ${completion}%`,
            );
          }
        }
      }
  ...
  const parseExpensesFromOutput = (output: string): { cost: number }[] => {
    ...
    console.log(
      `[PROJECTS] Total expenses found: ${expenses.length}, Total amount: $${totalAmount}`,
    );
    return expenses;
  };

  const _calculateSpentFromExpenses = (expensesOutput: string): number => {
    let total = 0;
  ...
  const generateTimeline = (status: string, created: string): string => {
    ...
    if (status.toLowerCase().includes("planning"))
      return `Started ${daysSince} days ago`;
    if (status.toLowerCase().includes("progress"))
      return `In progress (${daysSince} days)`;
  ...
  const _getPriorityColor = (priority: "high" | "medium" | "low" | "urgent") => {
    switch (priority) {
      case "urgent":
        return "bg-red-600";
      case "high":
  ...
  const handleStatusUpdate = async (projectId: number, newStatus: string) => {
    try {
      console.log(
        `[PROJECT] 🔄 Starting status update for project ${projectId}`,
      );
      ...
      const result = await TauriService.updateProjectStatus(
        projectId,
        newStatus,
      );
      ...
      console.log(
        `[PROJECT] 🎯 Successfully updated status for project ${projectId} to "${newStatus}"`,
      );
      ...
      console.error(
        `[PROJECT] ❌ Failed to update status for project ${projectId}:`,
        error,
      );
  ...
  const handlePriorityUpdate = async (
    ...
    try {
      console.log(
        `[PROJECT] Updating priority for project ${projectId} to: "${newPriority}"`,
      );
      ...
      setProjects((prevProjects) =>
        prevProjects.map((project) =>
          project.id === projectId
            ? {
                ...project,
                priority: newPriority as "high" | "medium" | "low" | "urgent",
              }
            : project,
        ),
      );
      ...
      console.log(
        `[PROJECT] Successfully updated priority for project ${projectId} to "${newPriority}"`,
      );
      toast.success(`Priority updated to ${newPriority}`);
    } catch (error) {
      console.error(
        `[PROJECT] Failed to update priority for project ${projectId}:`,
        error,
      );
  ...
  const statusFilterOptions = [
    ...
    },
    {
      value: "progress",
      label: "In Progress",
      description: "Active renovation projects",
    },
          ...
          {isLoading ? (
                      ...
                      >
                          ...
                          <div className="flex items-center">
                            ...
                            <div>
                              ...
                              <div className="text-sm text-gray-500 dark:text-gray-400">
                                ID: {project.id} • {project.rooms} rooms
                              </div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <QuickEditDropdown
                            ...
                            onSave={(newPriority) =>
                              handlePriorityUpdate(project.id, newPriority)
                            }
                            type="priority"
                          />
                        </td>
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                              <div
                                ...
                                style={{
                                  width: `${Math.min(project.completion, 100)}%`,
                                }}
                              ></div>
                            </div>
                          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredProjects.map((project) => (
                <div
                  key={project.id}
                  className="bg-white dark:bg-[#131619] rounded-lg border border-gray-200 dark:border-black p-6 hover:shadow-lg transition-shadow"
                >
                  {/* Project Header */}
