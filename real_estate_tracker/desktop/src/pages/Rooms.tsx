...
export default function Rooms() {
  ...
  const [formData, setFormData] = useState<Omit<Room, 'id' | 'projectName' | 'squareFootage'>>({
    name: '',
    floor: 1,
    length: undefined,
    width: undefined,
    ...
    for (const line of lines) {
      if (line.startsWith('│') && !line.includes('Name') && !line.includes('━')) {
        ...
        if (columns.length >= 4) {
          ...
          const condition = conditionMatch ? parseInt(conditionMatch[1], 10) : 3
  ...
  const handleDelete = async (room: Room) => {
    if (!window.confirm(`Are you sure you want to delete room "${room.name}"? This will also delete all expenses associated with this room.`)) {
      return
    }
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={() => selectedProjectId && loadRooms(selectedProjectId)}
            ...
            <div className="flex items-center gap-4 flex-1">
              <div className="min-w-64">
                ...
                <ScrollableSelect
                  ...
                  onChange={(value) => setSelectedProjectId(parseInt(value, 10))}
            ...
            <div className="text-sm text-gray-600 dark:text-gray-400">
              {selectedProject && (
                <>
                  ...
                  <br />
                  <span>{filteredRooms.length} room{filteredRooms.length !== 1 ? 's' : ''} found</span>
                </>
              )}
            </div>
          </div>
        ...
        {!selectedProjectId ? (
          <div className="bg-white dark:bg-[#131619] rounded-lg border border-gray-200 dark:border-black p-12 text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              Choose a project from the dropdown above to view and manage its rooms.
            </p>
          </div>
        ) : isLoading ? (
          <div className="bg-white dark:bg-[#131619] rounded-lg border border-gray-200 dark:border-black p-8">
            <div className="flex items-center justify-center">
              ...
              <span className="ml-3 text-gray-600 dark:text-gray-300">Loading rooms...</span>
            </div>
          </div>
              ...
              <table className="w-full">
                ...
                <tbody className="bg-white dark:bg-[#131619] divide-y divide-gray-200 dark:divide-black">
                  {filteredRooms.map((room, index) => (
                    <tr key={`${room.projectId}-${room.name}`} className={`hover:bg-gray-50 dark:hover:bg-[#171b1f] transition-colors ${
                      index !== filteredRooms.length - 1 ? 'border-b border-gray-200 dark:border-black' : ''
                    }`}>
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-900 dark:text-white">
                          {room.squareFootage ? `${room.squareFootage} sq ft` : 'Not set'}
                        </div>
                      </td>
                      <td className="px-6 py-4">
            ...
            <div className="bg-white dark:bg-[#131619] rounded-lg border border-gray-200 dark:border-black shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
              <div className="p-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                      Floor * <span className="text-xs text-gray-500">(Required)</span>
                    </label>
                    <input
                      type="number"
