...
class DataCacheService {
  ...
  setExpenses(projectId: number, expenses: ExpenseData[]): void {
    ...
    console.log(
      `[CACHE] Cached ${expenses.length} expenses for project ${projectId}`,
    );
  }
  ...
  getExpenses(projectId: number): ExpenseData[] | null {
    ...
    if (cached) {
      console.log(
        `[CACHE] Retrieved ${cached.length} expenses from cache for project ${projectId}`,
      );
    }
    return cached;
  }
  ...
  invalidateExpenses(projectId?: number): void {
    if (projectId) {
      ...
      console.log(
        `[CACHE] Invalidated expenses cache for project ${projectId}`,
      );
    } else {
  ...
  setRooms(projectId: number, rooms: RoomData[]): void {
    ...
    console.log(
      `[CACHE] Cached ${rooms.length} rooms for project ${projectId}`,
    );
  }
  ...
  batchSetProjects(
    ...
    projectExpenses: Map<number, ExpenseData[]>,
  ): void {
    // Set projects
