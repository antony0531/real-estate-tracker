...
class PerformanceOptimizerService {
  ...
  async loadProjectsOptimized(): Promise<ProjectDetails[]> {
    ...
    try {
      ...
      if (cachedProjects && cachedProjects.length > 0) {
        console.log(
          `[PERF] Cache hit! Loaded ${cachedProjects.length} projects in ${(performance.now() - startTime).toFixed(1)}ms`,
        );
      ...
      const detailPromises = basicProjects.map(async (project) => {
        try {
          ...
          console.warn(
            `[PERF] Failed to load details for project ${project.id}:`,
            error,
          );
          return {
            ...project,
      ...
      const successfulProjects = detailedProjects
        .filter(
          (result): result is PromiseFulfilledResult<ProjectDetails> =>
      ...
      console.log(
        `[PERF] Loaded ${successfulProjects.length} projects with full details in ${loadTime.toFixed(1)}ms`,
      );
  ...
  private parseBasicProjects(
    output: string,
  ): Omit<ProjectDetails, "spent" | "rooms" | "completion">[] {
  ...
  async preloadProjectDetails(projectIds: number[]): Promise<void> {
    console.log(
      `[PERF] Preloading details for ${projectIds.length} projects in background`,
    );
  ...
  private parseExpensesForCache(_output: string): ExpenseData[] {
    // Simplified parsing for cache
    return [];
  }

  private parseRoomsForCache(_output: string): unknown[] {
    // Simplified parsing for cache
    return [];
  }
